package com.threeDBJ.calcAppLib.graph;
...
public class Graph {
    ...
    public boolean xAxis() {
        return yorig >= 0 && yorig < ypix;
    }

    public boolean yAxis() {
        return xorig >= 0 && xorig < xpix;
    }
    ...
    */
    ...
    public float[] getXTicks() {
        ...
        for(; x < xpix; x += tickDist) {
            if(i >= xticks.length) {
                Log.v("xticks", Float.toString(tickDist) + " " + Integer.toString(i));
            }
            xticks[i] = x;
            xticks[i + 1] = 0;
            xticks[i + 2] = x;
    ...
    public float[] getYTicks() {
        ...
        for(; y < ypix; y += tickDist) {
            if(i >= yticks.length) {
                Log.v("yticks", Float.toString(tickDist) + " " + Integer.toString(i));
            }
            yticks[i] = 0;
            yticks[i + 1] = y;
            yticks[i + 2] = xpix;
